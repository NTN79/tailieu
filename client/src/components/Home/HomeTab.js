...
export default function HomeTab() {
    ...
    if (dateProductVirtual) {
        dateProductVirtual.sort((a,b)=> new Date(b.updatedAt) - new Date(a.updatedAt));
        for (let i in dateProductVirtual) {
            const today = new Date();
            const productDate = new Date(dateProductVirtual[i].updatedAt);
    ...
    return(
        <div className="HomeTab">
            <div className="home-tab flex-center">
                <p onClick={() => {setCurrentTab(1); setIsActive(1)}} className={isActive === 1 ? "home-tab-active" : ""}>Tất Cả Sản Phẩm</p>
                <p onClick={() => {setCurrentTab(2); setIsActive(2)}} className={isActive === 2 ? "home-tab-active" : ""}>Sản Phẩm Mới</p>
                <p onClick={() => {setCurrentTab(3); setIsActive(3)}} className={isActive === 3 ? "home-tab-active" : ""}>Sales</p>
            </div>
